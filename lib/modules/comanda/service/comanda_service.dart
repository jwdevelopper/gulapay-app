...
class ComandaService {
  ...
  Future<ComandaResponse> criar(ComandaCreateRequest request) => _mutate(
    () async => ComandaResponse.fromJson(
      Map<String, dynamic>.from(
        (await _dio.post(ConstantsApi.urlComandas, data: request.toJson())).data
            as Map,
      ),
    ),
  );

  Future<List<ComandaResponse>> listar({
    String? status,
    String? tipoOrigem,
    int? mesaId,
    int? clienteId,
    ...
    try {
      ...
      return (list as List)
          .whereType<Map>()
          .map((e) => ComandaResponse.fromJson(Map<String, dynamic>.from(e)))
          .toList();
  ...
  Future<ComandaResponse> patch(int id, ComandaPatchRequest request) => _mutate(
    () async => ComandaResponse.fromJson(
      Map<String, dynamic>.from(
        (await _dio.patch(
              '${ConstantsApi.urlComandas}/$id',
              data: request.toJson(),
            )).data
            as Map,
      ),
  ...
  Future<ComandaResponse> _get(String path) => _mutate(
    () async => ComandaResponse.fromJson(
      Map<String, dynamic>.from(
        (await _dio.get('${ConstantsApi.urlComandas}/$path')).data as Map,
      ),
    ),
  );
  Future<ComandaResponse> _action(int id, String action) => _mutate(
    () async => ComandaResponse.fromJson(
      Map<String, dynamic>.from(
        (await _dio.post('${ConstantsApi.urlComandas}/$id/$action')).data
            as Map,
      ),
    ),
  );
  Future<ComandaResponse> _mutate(
    Future<ComandaResponse> Function() call,
  ) async {
    try {
      return await call();
    } on DioException catch (e) {
