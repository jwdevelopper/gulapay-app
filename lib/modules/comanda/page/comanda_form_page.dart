...
class _ComandaFormPageState extends State<ComandaFormPage> {
  ...
  Widget _revisaoStep() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_title('Revise sua comanda', 'Confira os dados antes de abrir a venda.'), const SizedBox(height: 18), _summaryCard(), const SizedBox(height: 18), _label('Observação (opcional)'), const SizedBox(height: 8), _textField(_observacao, 'Inclua detalhes importantes do pedido', maxLines: 4)]);
  ...
  Widget _dropdownCliente({bool optional = false}) => DropdownButtonFormField<int>(initialValue: _cliente?.id, isExpanded: true, decoration: _decoration(optional ? 'Selecione um cliente ou deixe vazio' : 'Selecione um cliente'), items: _clientes.where((c) => c.id != null).map((c) => DropdownMenuItem(value: c.id, child: Text(c.nome ?? 'Cliente ${c.id}', overflow: TextOverflow.ellipsis))).toList(), onChanged: (value) => setState(() { _cliente = value == null ? null : _clientes.firstWhere((c) => c.id == value); _validationMessage = null; }));
  InputDecoration _decoration(String hint) => InputDecoration(hintText: hint, hintStyle: const TextStyle(color: EstoquePalette.textMuted), floatingLabelBehavior: FloatingLabelBehavior.always, filled: true, fillColor: EstoquePalette.surfaceAlt, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 15), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: EstoquePalette.border)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: EstoquePalette.border)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: EstoquePalette.primary, width: 1.5)));
