...
class _ComandaFormPageState extends State<ComandaFormPage> {
  ...
  Widget _header() => Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 8), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Nova comanda', style: TextStyle(color: PaletaApp.text, fontSize: 20, fontWeight: FontWeight.w700, height: 1.05)),
          const SizedBox(height: 2), Text('Etapa ${_step + 1} de 4 • $_stepLabel', style: const TextStyle(color: PaletaApp.textMuted, fontSize: 12)),
        ])),
      ]));

  Widget _iconButton(IconData icon, VoidCallback onTap) => Material(color: PaletaApp.surface, borderRadius: BorderRadius.circular(16), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16), child: Container(width: 44, height: 44, decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: PaletaApp.border)), child: Icon(icon, color: PaletaApp.text))));
  Widget _progress() => Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Row(children: List.generate(4, (index) => Expanded(child: Container(height: 4, margin: EdgeInsets.only(right: index == 3 ? 0 : 8), decoration: BoxDecoration(color: index <= _step ? PaletaApp.primary : PaletaApp.borderSoft, borderRadius: BorderRadius.circular(999)))))));
  ...
  Widget _title(String title, String subtitle) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: PaletaApp.text, fontSize: 20, fontWeight: FontWeight.w700)), const SizedBox(height: 6), Text(subtitle, style: const TextStyle(color: PaletaApp.textMuted, fontSize: 13, height: 1.35))]);
  Widget _label(String label) => Text(label, style: const TextStyle(color: PaletaApp.text, fontSize: 13, fontWeight: FontWeight.w700));
  ...
  Widget _canalCard(_CanalOption option) { final selected = option.value == _tipo; return Padding(padding: const EdgeInsets.only(bottom: 10), child: InkWell(onTap: () => setState(() { _tipo = option.value; _validationMessage = null; }), borderRadius: BorderRadius.circular(18), child: AnimatedContainer(duration: const Duration(milliseconds: 180), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: selected ? PaletaApp.inputFill : PaletaApp.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: selected ? PaletaApp.primary : PaletaApp.border, width: selected ? 1.5 : 1), boxShadow: const [BoxShadow(color: PaletaApp.sombraCampo, blurRadius: 12, offset: Offset(0, 4))]), child: Row(children: [Container(width: 44, height: 44, decoration: BoxDecoration(color: selected ? PaletaApp.primary : PaletaApp.inputFill, borderRadius: BorderRadius.circular(14)), child: Icon(option.icon, color: selected ? Colors.white : PaletaApp.primary)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(option.label, style: const TextStyle(color: PaletaApp.text, fontWeight: FontWeight.w700, fontSize: 15)), const SizedBox(height: 3), Text(option.description, style: const TextStyle(color: PaletaApp.textMuted, fontSize: 12))])), Icon(selected ? Icons.radio_button_checked_rounded : Icons.radio_button_unchecked_rounded, color: selected ? PaletaApp.primary : PaletaApp.textMuted)])))); }
  ...
  Widget _clientCard() => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: PaletaApp.inputFill, borderRadius: BorderRadius.circular(16), border: Border.all(color: PaletaApp.border)), child: Row(children: [const Icon(Icons.person_outline_rounded, color: PaletaApp.primary), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_cliente!.nome ?? 'Cliente selecionado', style: const TextStyle(color: PaletaApp.text, fontWeight: FontWeight.w700)), if (_cliente!.telefone?.isNotEmpty == true) Text(_cliente!.telefone!, style: const TextStyle(color: PaletaApp.textMuted, fontSize: 12))]))]));
  ...
  Widget _textField(TextEditingController controller, String hint, {int maxLines = 1}) => Container(decoration: DecoracoesApp.campo(), child: TextField(controller: controller, maxLines: maxLines, style: const TextStyle(color: PaletaApp.text, fontSize: 15), decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(color: PaletaApp.textMuted), border: InputBorder.none, contentPadding: const EdgeInsets.all(16))));
  Widget _addressCard() => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: _cliente?.endereco == null ? PaletaApp.warningBg : PaletaApp.inputFill, borderRadius: BorderRadius.circular(16), border: Border.all(color: _cliente?.endereco == null ? PaletaApp.warningBorder : PaletaApp.border)), child: Row(children: [Icon(_cliente?.endereco == null ? Icons.info_outline_rounded : Icons.location_on_outlined, color: PaletaApp.primary, size: 20), const SizedBox(width: 10), Expanded(child: Text(_cliente?.endereco == null ? 'O cliente precisa ter endereço cadastrado.' : 'Endereço cadastrado selecionado para a entrega.', style: const TextStyle(color: PaletaApp.text, fontSize: 12, fontWeight: FontWeight.w600, height: 1.35)))]));
  Widget _infoCard(String text) => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: PaletaApp.warningBg, borderRadius: BorderRadius.circular(16), border: Border.all(color: PaletaApp.warningBorder)), child: Row(children: [const Icon(Icons.lightbulb_outline_rounded, color: PaletaApp.primary, size: 20), const SizedBox(width: 10), Expanded(child: Text(text, style: const TextStyle(color: PaletaApp.text, fontSize: 12, fontWeight: FontWeight.w600, height: 1.35)))]));
  Widget _errorBanner() => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: PaletaApp.warningBg, borderRadius: BorderRadius.circular(16), border: Border.all(color: PaletaApp.error)), child: Row(children: [const Icon(Icons.warning_amber_rounded, color: PaletaApp.error, size: 18), const SizedBox(width: 10), Expanded(child: Text(_validationMessage!, style: const TextStyle(color: PaletaApp.text, fontSize: 12, fontWeight: FontWeight.w600)))]));
  ...
  Widget _summaryCard() => Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: PaletaApp.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: PaletaApp.border)), child: Column(children: [_summaryLine('Canal', _canalOptions.firstWhere((o) => o.value == _tipo).label), if (_mesaId != null) _summaryLine('Mesa', _mesaLabel), if (_garcomId != null) _summaryLine('Garçom', _garcomLabel), if (_cliente != null) _summaryLine('Cliente', _cliente!.nome ?? 'Cliente'), if (_tipo == 'MESA') _summaryLine('Escopo', _escopo == 'COMPARTILHADA' ? 'Compartilhada' : 'Individual')]));
  Widget _summaryLine(String label, String value) => Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: const TextStyle(color: PaletaApp.textMuted, fontSize: 13)), Flexible(child: Text(value, textAlign: TextAlign.end, style: const TextStyle(color: PaletaApp.text, fontSize: 13, fontWeight: FontWeight.w700)))]));
  Widget _bottomButtons() => SafeArea(top: false, child: Padding(padding: const EdgeInsets.fromLTRB(16, 4, 16, 12), child: Row(children: [Expanded(child: OutlinedButton(onPressed: _previous, style: OutlinedButton.styleFrom(foregroundColor: PaletaApp.text, backgroundColor: PaletaApp.surfaceAlt, side: const BorderSide(color: PaletaApp.border), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), padding: const EdgeInsets.symmetric(vertical: 16)), child: Text(_step == 0 ? 'Cancelar' : 'Voltar'))), const SizedBox(width: 12), Expanded(child: ElevatedButton(onPressed: _saving ? null : _next, style: ElevatedButton.styleFrom(backgroundColor: PaletaApp.primary, foregroundColor: Colors.white, disabledBackgroundColor: PaletaApp.primarySoft, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), padding: const EdgeInsets.symmetric(vertical: 16)), child: _saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text(_step == 3 ? 'Abrir comanda' : 'Continuar'), const SizedBox(width: 8), const Icon(Icons.chevron_right_rounded, size: 18)]))) ])));
}
