...
const List<_ChoiceOption> _tipoOptions = [
  _ChoiceOption(
    label: 'Compra',
    description: 'Entrada de fornecedor',
    value: 'ENTRADA_COMPRA',
    icon: Icons.shopping_cart_rounded,
  ),
  _ChoiceOption(
    label: 'Perda validade',
    description: 'Insumo vencido',
    value: 'SAIDA_PERDA_VALIDADE',
    icon: Icons.timer_off_rounded,
  ),
  _ChoiceOption(
    label: 'Perda quebra',
    description: 'Quebra/avaria',
    value: 'SAIDA_PERDA_QUEBRA',
    icon: Icons.broken_image_rounded,
  ),
  _ChoiceOption(
    label: 'Troca',
    description: 'Entrada por troca',
    value: 'ENTRADA_TROCA',
    icon: Icons.swap_horiz_rounded,
  ),
  _ChoiceOption(
    label: 'Ajuste de inventário',
    description: 'Corrige saldo pós-contagem física',
...
class _MovimentacaoFormPageState extends State<MovimentacaoFormPage> {
  ...
  final _moedaFormatter = CurrencyTextInputFormatter.currency(
    locale: 'pt_BR',
    symbol: 'R\$ ',
    decimalDigits: 2,
  );
  ...
  bool get _isEntrada =>
      _selectedTipo == 'ENTRADA_COMPRA' || _selectedTipo == 'ENTRADA_TROCA';
  ...
  double _parseQty() =>
      double.tryParse(_quantidade.text.replaceAll(',', '.')) ?? 0;
  double _parseCusto() => _custoUnitario.text.isEmpty
      ? 0
      : _moedaFormatter.getUnformattedValue().toDouble();
  ...
  bool _validateStep() {
    ...
    if (_step == 0) {
      if (_selectedTipo == null) {
        setState(() {
          _tipoError = true;
          _validationMessage = 'Selecione o tipo de movimentação.';
        });
        return false;
      }
      return true;
                ...
                children: [
                  ...
                  Expanded(
                    child: unidades.isEmpty
                        ? const Center(
                            child: Text(
                              'Nenhuma unidade compatível cadastrada',
                              style: TextStyle(
                                color: EstoquePalette.textMuted,
                                fontSize: 14,
  ...
  Widget _buildTextField({
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: error ? EstoquePalette.error : EstoquePalette.border,
        ),
        boxShadow: const [
          BoxShadow(
  ...
  Widget _buildErrorBanner() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: EstoquePalette.error),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: selected ? EstoquePalette.primary : EstoquePalette.border,
          ),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(
                    option.icon,
                    color: selected ? Colors.white : EstoquePalette.primary,
                    size: 18,
                  ),
                ),
                const Spacer(),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: _insumoError ? EstoquePalette.error : EstoquePalette.border,
          ),
          boxShadow: const [
            BoxShadow(
              color: Color(0x0F9C5A1E),
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 42,
              decoration: BoxDecoration(
                color: has ? EstoquePalette.primary : EstoquePalette.inputFill,
                borderRadius: BorderRadius.circular(14),
              ),
              child: Icon(
                Icons.inventory_2_rounded,
                color: has ? Colors.white : EstoquePalette.primary,
                size: 20,
              ),
            ),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    has ? _selectedInsumo!.nome : 'Selecione o insumo',
                    style: TextStyle(
                      color: has
                          ? EstoquePalette.text
                  ...
                  if (has)
                    Text(
                      'Saldo atual: ${_selectedInsumo!.estoqueAtual?.toStringAsFixed(1) ?? '0'} ${_selectedInsumo!.unidadePadraoSimbolo ?? ''}',
                      style: const TextStyle(
                        color: EstoquePalette.textMuted,
                        fontSize: 11,
                      ),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: _unidadeError ? EstoquePalette.error : EstoquePalette.border,
          ),
        ),
        child: Row(
          children: [
            Text(
              has
                  ? (_selectedUnidade!.simbolo ?? _selectedUnidade!.nome)
                  : 'Unidade',
              style: TextStyle(
                color: has ? EstoquePalette.text : EstoquePalette.textMuted,
                fontSize: 14,
                fontWeight: FontWeight.w700,
              ),
            ),
      ...
      children: [
        ...
        ..._lotes.map((lote) {
          ...
          return Padding(
            ...
            child: InkWell(
              onTap: () => setState(() => _selectedLote = lote),
              borderRadius: BorderRadius.circular(16),
              child: Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${lote.quantidadeRestante?.toStringAsFixed(1) ?? '0'} ${lote.unidadePadraoSimbolo ?? ''} restantes · custo R\$ ${lote.custoUnitario?.toStringAsFixed(2).replaceAll('.', ',') ?? '0'}',
                            style: const TextStyle(
                              color: EstoquePalette.textMuted,
                              fontSize: 11,
                            ),
      ...
      children: [
        ...
        if (_isEntrada) ...[
          ...
          TextFormField(
            ...
            onChanged: (_) {
              setState(() {}); // Garante que o Resumo atualize em tempo real
            },
          ),
          const SizedBox(height: 16),
          const Text(
        ...
        children: [
          ...
          if (_parseCusto() > 0) ...[
            ...
            _SummaryRow(
              ...
              value:
                  'R\$ ${_parseCusto().toStringAsFixed(2).replaceAll('.', ',')}',
            ),
          ],
        ],
      ),
  ...
  Widget _buildBottomButtons() {
    ...
    if (_step == 2) {
      if (_isEntrada) {
        rightLabel = 'Registrar compra';
      } else {
        rightLabel = 'Registrar saída';
      }
    } else {
      rightLabel = 'Continuar';
    }
            ...
            Expanded(
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: EstoquePalette.primary,
                  foregroundColor: Colors.white,
                  disabledBackgroundColor: EstoquePalette.primarySoft
                  ...
                  child: _insumosFiltrados.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final selected =
                                insumo.id == widget.insumoSelecionado?.id;
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          const SizedBox(height: 2),
                                          Text(
                                            'Saldo atual: ${insumo.estoqueAtual?.toStringAsFixed(1) ?? '0'} ${insumo.unidadePadraoSimbolo ?? ''}',
                                            style: const TextStyle(
                                              color: EstoquePalette.textMuted,
                                              fontSize: 12,
                                            ),
