...
class RestaurantTable {
  ...
  RestaurantTable copyWith({
    ...
    return RestaurantTable(
      ...
      seatedPeople: clearSeatedPeople
          ? null
          : (seatedPeople ?? this.seatedPeople),
...
class TableOriginalPosition {
  ...
  TableOriginalPosition copyWith({String? tableId, double? x, double? y}) {
  ...
  Map<String, dynamic> toMap() {
    return {'tableId': tableId, 'x': x, 'y': y};
  }
...
class RestaurantArea {
  ...
  int get occupancyCount => tables
      .where(
        (table) => table.activeOrderId != null || (table.seatedPeople ?? 0) > 0,
      )
      .length;
  ...
  factory RestaurantArea.fromMap(Map<String, dynamic> map) {
    return RestaurantArea(
      ...
      joinGroups: (map['joinGroups'] as List<dynamic>? ?? const [])
          .map(
            (group) =>
                TableJoinGroup.fromMap(Map<String, dynamic>.from(group as Map)),
          )
          .toList(),
    );
  }
...
class FloorPlanSnapshot {
  FloorPlanSnapshot({required this.selectedAreaId, required this.areas});
  ...
  factory FloorPlanSnapshot.fromMap(Map<String, dynamic> map) {
    return FloorPlanSnapshot(
      ...
      areas: (map['areas'] as List<dynamic>)
          .map(
            (area) =>
                RestaurantArea.fromMap(Map<String, dynamic>.from(area as Map)),
          )
          .toList(),
    );
  }
