...
class _CategoriaListaPageState extends State<CategoriaListaPage> {
  ...
  Future<void> _carregar() async {
    ...
    try {
      final lista = await _servico.listarCategorias();
      if (!mounted) return;
  ...
  List<CategoriaDto> get _filtradas {
  ...
  Future<bool> _confirmarMudancaStatus(CategoriaDto c, bool inativar) async {
  ...
  Future<bool> _alternarStatus(CategoriaDto c, bool inativar) async {
    if (c.id == null) return false;
    try {
      if (inativar) {
        await _servico.inativarCategoria(c.id!);
        ...
        final atualizada = await _servico.ativarCategoria(c);
        if (!mounted) return false;
        setState(() {
          final idx = _todas.indexWhere((x) => x.id == c.id);
  ...
  Future<void> _abrirFormulario({CategoriaDto? categoria}) async {
  ...
  Widget _construirCartao(CategoriaDto c) {
    final ativa = c.ativo ?? true;
